...
interface credentials extends  Partial<user> {
    type: "credentials",
    //
...
//
//A dialog-based class that is to be exported as a module in order to
//access registration services
export class registration extends dialog<user>{
    //
    ...
    constructor(anchor:HTMLElement){
        //
        //Initialize an instance of the dialog class
        super({url:"/registration/v/code/registration.html",anchor});
    }
    //
    //
    public save(input:user):Promise<"ok" | Error>{
        //
        //Test if the user is new or old. If new, sign up; otherwise sign in
        //
        //Define the user that we will eventually to return
        let new_user:user|Error;
        //
        //Check if the user forgot password
        if(operation ==='forgot'){};
        //
        //Create the outlook provider to access sign-up or sign-in services
    ...
    private get_user():raw<{username: string; email: string}>{
        //
